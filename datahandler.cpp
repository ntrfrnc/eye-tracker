...
// funkcja otwierająca port
HANDLE openPort(string port) {
  HANDLE serial = CreateFile(port.c_str(), GENERIC_READ, 0, 0, OPEN_EXISTING,
                             FILE_ATTRIBUTE_NORMAL, 0);

  return serial;
}

// funkcja ustawiająca parametry połaczenia
bool DataHandler::setParams(int baudRate, int byteSize, int stopBits,
                            int parity) {
  DCB serialParams;
  bool error = false;

  // pobranie ustawień portu
  if (!GetCommState(*serial, &serialParams)) error = true;

  // przypisanie wartości do struktury
  serialParams.BaudRate = baudRate;
  serialParams.ByteSize = byteSize;
  serialParams.StopBits = stopBits;  // error
  serialParams.Parity = parity;

  // przypisanie ustawień do portu
  if (!SetCommState(*serial, &serialParams)) error = true;

  //    cout<<"BaudRate set to "<<(int)serialParams.BaudRate<<endl;
  //    cout<<"ByteSize set to "<<(int)serialParams.ByteSize<<endl;
  //    cout<<"StopBits set to "<<(int)serialParams.StopBits<<endl;
  //    cout<<"Parity set to "<<(int)serialParams.Parity<<endl;

  return !error;
}

// funkcja synchronizująca dane
bool DataHandler::synchronize() {
  char buff[1];
  DWORD bytesRead = 0;
  int counter = 0;

  while (true)
    if (!ReadFile(*serial, buff, 1, &bytesRead, NULL))
      return false;
    else if (buff[0] == 0) {
      counter++;
...
Packet DataHandler::getPacket() {
  ...
  // synchronizacja i odczyt danych
//  if (!synchronize(serial))
//    cout << "ERROR: Failed to synchronize data" << endl;
//  else if (!ReadFile(*serial, buff, PACKET - 3, &bytesRead, NULL))
//    cout << "ERROR: Failed to write data" << endl;
//  else
//    cout << "Packet received" << endl;

  merge.append(buff, PACKET - 3);

  // inicjalizacja sygnałów pakietu
...
// funkcja odbierająca ciąg pakietów
void DataHandler::streamPackets(Packet *buff, int *packetsRead) {
  int counter;

  //    cout<<"Press any key to start acquisition"<<endl<<"Press ESC to finish
  //    acquisition"<<endl;
  getch();

  for (counter = 0; counter < BUFF; counter++) {
    buff[counter] = getPacket(serial);
    if (kbhit() && getch() == 27) break;
  }

//  cout << "Acquisition stopped, " << counter << " packets recived" << endl;
  *packetsRead = counter;
}

// funkcja zapisująca pakiety z bufora do pliku
void DataHandler::datasaveOutput(Packet *buff, int packetsRead) {
  ofstream file;
